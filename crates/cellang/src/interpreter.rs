...
pub fn eval_ast<'src>(runtime: &Runtime, node: &TokenTree<'src>) -> Result<Value, RuntimeError> {
	match node {
...
fn eval_atom(runtime: &Runtime, atom: &Atom<'_>) -> Result<Value, RuntimeError> {
	Ok(match atom {
...
fn eval_cons(
	...
	match op {
		Op::Group => ensure_arity(op, args, 1).and_then(|_| eval_ast(runtime, &args[0])),
		Op::List => eval_list(runtime, args),
		Op::Map => eval_map(runtime, args),
		Op::Dyn => ensure_arity(op, args, 1).and_then(|_| eval_ast(runtime, &args[0])),
		Op::Not => {
			ensure_arity(op, args, 1)?;
			let value = eval_ast(runtime, &args[0])?;
			let flag = <bool as TryFromValue>::try_from_value(&value)
		...
		Op::EqualEqual | Op::NotEqual => {
			...
			let right = eval_ast(runtime, &args[1])?;
			let eq = left == right;
			Ok(Value::Bool(if matches!(op, Op::EqualEqual) { eq } else { !eq }))
		}
		...
		Op::IfTernary => {
			if args.len() != 3 {
				return Err(RuntimeError::new("ternary operator requires condition and two branches"));
			}
		...
		Op::Call | Op::For | Op::Var | Op::While => Err(RuntimeError::new(format!(
			"Operator '{op:?}' is not supported in this context"
		))),
	}
}

fn eval_list(runtime: &Runtime, args: &[TokenTree<'_>]) -> Result<Value, RuntimeError> {
...
fn eval_map(runtime: &Runtime, args: &[TokenTree<'_>]) -> Result<Value, RuntimeError> {
	if args.len() % 2 != 0 {
...
fn eval_call(
	...
	let name = resolve_function_name(func)
		.ok_or_else(|| RuntimeError::new("function calls must reference an identifier"))?;
	let handler = runtime
		.lookup_function(&name)
		.ok_or_else(|| RuntimeError::new(format!("Function '{name}' is not registered")))?;
...
fn eval_field(
	...
	let field_name = match field_expr {
		...
		_ => {
			...
			<String as TryFromValue>::try_from_value(&value)
				.map_err(|err| RuntimeError::with_source("field access expects identifier", err))?
		}
	};

	match target {
		Value::Struct(strct) => strct
			.get(&field_name)
			.cloned()
...
fn eval_in_operator(needle: &Value, haystack: &Value) -> Result<Value, RuntimeError> {
	let result = match haystack {
		...
		Value::String(text) => {
			let query = <String as TryFromValue>::try_from_value(needle)
				.map_err(|err| RuntimeError::with_source("expected string", err))?;
			text.contains(&query)
		}
		Value::Bytes(bytes) => {
			let query = <Vec<u8> as TryFromValue>::try_from_value(needle)
				.map_err(|err| RuntimeError::with_source("expected bytes", err))?;
			bytes.windows(query.len()).any(|window| window == query.as_slice())
		}
		_ => {
	...
	match target {
		...
		Value::String(text) => {
			...
			let ch = text
				.chars()
				.nth(idx)
				.ok_or_else(|| RuntimeError::new("string index out of bounds"))?;
		...
		Value::Struct(strct) => {
			let field = <String as TryFromValue>::try_from_value(index)
				.map_err(|err| RuntimeError::with_source("struct index expects field name", err))?;
			strct
				.get(&field)
				.cloned()
				.ok_or_else(|| RuntimeError::new(format!(
...
fn compare_numeric(left: Number, right: Number) -> Result<Ordering, RuntimeError> {
	match (left, right) {
		(Number::Int(a), Number::Int(b)) => Ok(a.cmp(&b)),
		(Number::Uint(a), Number::Uint(b)) => Ok(a.cmp(&b)),
		(Number::Double(a), Number::Double(b)) => a
			.partial_cmp(&b)
...
fn ensure_arity(op: Op, args: &[TokenTree<'_>], expected: usize) -> Result<(), RuntimeError> {
